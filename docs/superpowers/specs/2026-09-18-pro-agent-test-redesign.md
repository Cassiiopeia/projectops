...
**쓰는 사람이 아직 없다.** 옛 것을 붙잡고 가면 코드가 두 벌이 되고, 그 상태가 오래 간다.

- 폐기 안내(`_RETIRED`)를 두지 않는다 — 없어진 것은 그냥 없다
- 옛 산출물 위치(`docs/testing/e2e`)에서 홈으로 옮겨 오는 이전 로직을 두지 않는다.
  홈이 처음부터 정본이다
- 옛 이름(`pro-flutter-e2e`) 마커를 알아보지 않는다
- `mode` 축을 두지 않는다. 부하 테스트를 하지 않기로 했으므로 축 자체가 필요 없다

`target`의 기본값(`app`)만 남긴다 — 이건 호환이 아니라 **대부분이 앱이라 매번 적게 하지
않으려는 것**이다.
...
| 갈래 | 서브커맨드 |
|---|---|
| 판정 | `detect`(타겟) · `devices` · `doctor` |
| 실행 | `web` · `api` · `db` · `logs` |
| 기록 | `scenario` · `note` · `access` |
| 증적 | `shrink` |

**"프로젝트를 맞히는" 것은 하나도 남기지 않는다.** 그건 agent가 Grep·Read로 직접 한다.
