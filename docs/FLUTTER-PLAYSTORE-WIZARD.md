...
> ⚠️ **개인 계정이면 비공개 테스트가 사실상 필수다.**
> **2023-11-13 이후에 만든 개인 개발자 계정**은 테스터 12명 이상이 14일 이상 참여한
> **비공개 테스트**를 마쳐야 프로덕션 액세스를 받는다. 이 템플릿의 기본 배포는 **내부
> 테스트 트랙**까지인데, 내부 테스트는 그 조건에 **잡히지 않는다.**
>
> 따라서 개인 계정이라면 배포 설정에서 **비공개 테스트 승급을 켜 둔다.** 안 켜면
> 배포는 계속 나가는데 프로덕션 액세스 조건이 영영 차지 않고, 비공개 트랙만 손으로
> 올리게 되어 두 트랙의 버전이 벌어진다.
>
> 켜는 방법과 트랙별 성격은 [FLUTTER-CICD-OVERVIEW.md](FLUTTER-CICD-OVERVIEW.md) 의
> "Android 트랙" 절. (그 이전에 만든 계정·조직 계정은 해당하지 않는다.)
