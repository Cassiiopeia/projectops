...
> `references/looks-wrong.md` 의 축 4·5·6·7 이 그것이다 — 기능이 도는 것만 보면
...
## Phase 4.4 — 되는데 이상한 것을 본다 ⚠️

**기능이 도는 것만 보면 "되는데 이상한 것"은 전부 통과로 끝난다.** 폰트가 화면마다
다르거나, 눌리는데 아무 반응이 없거나, 화살표는 되는데 기기 뒤로가기는 막혀 있거나,
누를 것이 하나뿐인 화면이 끼어 있는 것들이다.

찍어 둔 스크린샷(`$SHOT_DIR`)을 나란히 놓고 **네 축으로 훑는다.**

| 축 | 묻는 것 |
| --- | --- |
| **4 — 같은 것이 같게 생겼나** | 같은 역할에 몇 종류의 크기·색·여백이 쓰였나 |
| **5 — 눌러서 말이 되나** | 반응 없는 요소 · 이유 모를 비활성 · 되돌아갈 길 · 불필요한 단계 |
| **6 — 움직임이 맞나** | 있어야 할 곳에 있나 · 끝나고 원위치로 돌아왔나 |
| **7 — 시안에서 온 화면인가** | 대조한 기록이 있나 · **에셋은 맞는데 다르게 보이지 않나** |

> **근거는 값으로만 적는다.** "폰트가 통일 안 된 느낌"이 아니라 **"제목 크기가 4개
> 화면에서 3종류"** 다. 이 축들은 "이 정도면 됐다"가 사람마다 달라서, 숫자로 묶어 두지
> 않으면 판정이 논쟁으로 끝난다. 구현한 쪽이 그대로 판정하면 통과시키고 싶은 방향으로
> 읽게 되는 것도 이 축에서 특히 심하다.

보는 법·코드에서 후보 뽑는 법·독립 판정 방법은 **`references/looks-wrong.md`**.

> **이 화면이 시안에서 왔다면 — 대조를 했는가?** 접근성 트리에는 그림자·색·굵기가
> 없어서 이 스킬로는 **시안 값이 전부 왔는지 볼 수 없다.** 시안이 있는 화면인데
> 대조한 기록이 없으면 **그 사실 자체를 결함으로 보고한다** (`pro-figma-verify`).
> 실제로 효과 네 줄 중 세 줄이 빠진 채 배포되고, 디자이너가 배포 후에 발견한 적이 있다.
