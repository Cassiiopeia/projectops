---
name: pro-agent-test
description: "앱·웹·서버를 agent가 직접 조작해 끝까지 밟고 결과를 데이터로 증명한다. 유닛/위젯 테스트가 아니라 사람이 하듯 화면을 누르고(앱·웹) 요청을 보내며(서버), 단계마다 기기 저장소·브라우저 상태·서버 DB를 대조하고, 2단계 인증처럼 사람이 해야 하는 지점에서 멈추고 무엇을 해달라고 알려준다. 로그가 없어 상태를 확인할 수 없으면 로깅을 먼저 심는다. '실제로 테스트해줘', '에뮬레이터로 돌려봐', '시뮬레이터에서 확인해줘', '브라우저로 확인해줘', '웹에서 눌러봐', 'API 순서대로 밟아줘', 'E2E 해줘', '로그인부터 끝까지 밟아줘', '실기기 검증', '앱 실행해서 확인' 같은 요청에 사용한다. QA 문서를 만드는 일(pro-testcase)과는 다르다 — 이 skill은 실제로 실행한다."
version: "2.0"
---
