...
**쌓은 것은 프로젝트 밖(홈)에 둔다.** 예전에는 프로젝트 안 `docs/testing/e2e/`였는데,
그 폴더는 추적에서 빠져 있어 **새 워크트리에 복사되지 않았다.** 이슈마다 워크트리를 만드는
흐름에서는 이슈 하나가 끝날 때마다 쌓은 지식이 통째로 사라졌다 (#586).

프로젝트 구분은 **git remote의 `owner/repo`** 로 한다. 경로로 구분하면 워크트리마다
갈라져 같은 문제가 그대로 남는다.

예전 위치에 있던 것은 **처음 한 번 자동으로 옮겨 온다.** 옮겼다는 사실은 명령 결과의
`migrated` 필드로 알려주므로, 프로젝트 폴더에서 파일을 찾다가 없어서 당황할 일이 없다.

레포에 아무것도 남지 않으므로 테스트 계정·토큰·서버 주소가 공개 레포로 새는 경로가
구조적으로 사라진다 (#578에서 실제로 겪은 사고다).
...
이걸 잡으려면 "이 프로젝트는 실패를 흔들림으로 알린다"는 **규칙을 알고 있어야 한다.**
그래서 이 단계는 프로젝트를 아는 QA만 할 수 있고, 정확히 그래서 값어치가 있다.
...
| **파급** | 이걸 건드리면 어디가 영향받나 | 이 API·모델·설정·위젯을 쓰는 곳을 찾는다 |
...
1. 이번에 밟은 것에서 **규칙을 한 문장으로 뽑는다**
   — "실패는 색이 아니라 흔들림으로 알린다" · "삭제는 되돌릴 수 없으므로 확인을 받는다"
