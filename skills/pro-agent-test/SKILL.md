...
#### 2단계 인증에서 멈춘다 — 무엇을 누를지까지 적어 준다

`/signin/challenge/` 에 닿으면 거기까지다. 사용자에게 넘기되 **화면에서 기기 이름과
숫자를 읽어 그대로 전달한다.** 이게 빠지면 폰을 들고도 무엇을 눌러야 할지 모른다.

```
지금 해주셔야 합니다 — 구글 2단계 인증

{기기명}으로 알림이 갔습니다. "예"를 누른 뒤 {숫자}를 탭해 주세요.
지나가도 다시 시도하면 되니 괜찮습니다.
```

통과한 뒤 **복구 정보 설정 화면이 끼어들면 "취소"를 누른다** — 계정 설정을 임의로 바꾸지 않는다.
