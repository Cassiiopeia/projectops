...
description: "Figma 시안을 코드로 옮기고, 옮긴 것이 시안과 같은지 센다. 덤프의 fills·strokes·effects·layout 을 하나도 빠짐없이 나열해 작업 목록으로 삼고, 반응형 단위로 옮긴 뒤, 시안 export 와 앱 렌더를 픽셀로 맞대 어긋난 자리를 덩어리로 짚는다. 'figma 디자인 구현해줘', '시안대로 만들어줘', 'figma 코드로 바꿔줘', '시안이랑 같은지 확인해줘', '디자인대로 됐는지 봐줘', 'figma 대조해줘', '그림자 빠진 거 없나' 같은 요청에 사용한다. 앱을 밟아 동작 결함을 찾는 것은 pro-agent-test 다."
---
